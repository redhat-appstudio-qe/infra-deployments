...
The resources here, which run in the `openshift-pipelines` namespace, help manage RBAC permissions and expose metrics regarding pipeline runs and the Tekton Chains controller. The namespace is created in this component and the `konflux-pipeline-service`/`konflux-sre` groups are granted `edit` access.

Tekton Chains' signing key and public key are synced in from Vault (`appsre-stonesoup-vault` ClusterSecretStore); the public key is readable by all authenticated users. A Pipeline Metrics Exporter `ServiceAccount` and its `Service`/`ServiceMonitor` expose PipelineRun/TaskRun metrics, alongside a `ServiceMonitor` for the Tekton Chains controller. The ArgoCD application controller is granted permission to manage the `ServiceMonitor`, `TektonConfig`, `SecurityContextConstraints`, and `batch/jobs` resources, since the operator doesn't own those directly.

### Tekton Logging

The resources here, which mainly exist in the `tekton-logging` namespace, sync the credentials needed to authenticate to the AWS Tekton Results S3 bucket and provides the `konflux-pipeline-service` / `konflux-sre` groups cluster-scoped access to debug resources and issues related to  cluster upgrades and cluster health. This subdirectory also contains the Pipelines Metrics Exporter `Deployment`.
