...
`rings/ring-0/base/cr/image-controller/image-controller.yaml` to inject Quay
push credentials (`QUAY_TOKEN`) so that image-controller can push built images
during E2E runs. This edit is local to the preview run and is not committed.
