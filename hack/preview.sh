...
[[ -n "${MULTI_PLATFORM_CONTROLLER_PR_OWNER}" && "${MULTI_PLATFORM_CONTROLLER_PR_SHA}" ]] && yq -i e "(.resources[] | select(. ==\"*github.com/redhat-appstudio/multi-platform-controller*\")) |= \"https://github.com/${MULTI_PLATFORM_CONTROLLER_PR_OWNER}/multi-platform-controller/config/default?ref=${MULTI_PLATFORM_CONTROLLER_PR_SHA}\"" $ROOT/components/multi-platform-controller/base/kustomization.yaml
