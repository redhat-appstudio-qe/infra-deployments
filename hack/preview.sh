...
deploy_and_wait_for_argocd() {
    ...
    while [ "$(oc get applications.argoproj.io all-application-sets -n $ARGOCD_NAMESPACE -o jsonpath='{.status.health.status} {.status.sync.status}')" != "Healthy Synced" ]; do
        log_wait "Root application 'all-application-sets' is not yet ready, checking again in 5s..."
        sleep 5
    done
    ...
    while [ -n "$(oc get applications.argoproj.io -n $ARGOCD_NAMESPACE -o jsonpath='{range .items[*]}{@.metadata.annotations.argocd\.argoproj\.io/refresh}{end}')" ]; do
        sleep 5
    done
    log_success "All refresh operations completed"
    ...
    while :; do
        ...
        log_progress "Iteration $iteration: $synced_apps/$total_apps applications synced and healthy ($pending_apps pending)"
        ...
        # Show detailed status of pending applications
        log_info "Pending applications status:"
        echo "$not_done" | while read -r line; do
            app_name=$(echo "$line" | awk '{print $1}')
            sync_status=$(echo "$line" | awk '{print $2}')
            health_status=$(echo "$line" | awk '{print $3}')
            log_debug "  - $app_name: sync=$sync_status, health=$health_status"
        done
