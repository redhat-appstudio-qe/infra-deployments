...
compare() {
  ...
  if ! rd_out="$(build "$rd_path" 2>/tmp/ring-err.$$)"; then
    echo "FAIL: $label"
    echo "      kustomize build failed: $rd_path"
    sed 's/^/        /' /tmp/ring-err.$$ >&2 || true
    rm -f /tmp/ring-err.$$
    FAIL=$((FAIL + 1))
    return
  fi
  if ! orig_out="$(build "$orig_path" 2>/tmp/ring-err.$$)"; then
    echo "FAIL: $label"
...
print_diff() {
  ...
  if [ "$n" -gt "$MAX_DIFF_LINES" ]; then
    printf '%s\n' "$text" | head -n "$MAX_DIFF_LINES" | sed 's/^/      /'
...
check_ring() {
  ...
  if [ "$found" -eq 0 ]; then
    echo "WARN: $ring - no cluster overlays found (only base/base-snapshot)"
  fi
}
