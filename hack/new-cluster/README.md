...
Operator-first bootstrap requires an existing operator ring base (`ring-1` or
`ring-2` today). Use `ring-1` for staging and `ring-2` for production until
later rings have operator bases. The cluster is also appended to the
konflux-operator ApplicationSet `nameNormalized In` selector in
...
* `ring` - One of `ring-1`, `ring-2`, `ring-3`, or `ring-4`. Operator-first
  bootstrap currently has konflux-operator bases for `ring-1` and `ring-2`
  only; other rings will fail until those bases exist.
