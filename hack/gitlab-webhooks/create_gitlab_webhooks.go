...
func main() {
	...
	if len(os.Args) != 2 {
		fmt.Println("script must be run with exactly one argument; the webhook URL")
		os.Exit(1)
	}

	var webhook_url = os.Args[1]

	fmt.Println("Searching for Repository resources with Git provider URL 'https://gitlab.com'...")

	// Get all repository resources with a git provider URL of 'https://gitlab.com'.
	cmd := exec.Command("bash", "-c", `oc get repository -A -o json | jq -c '.items[] | select(.spec.git_provider.url == "https://gitlab.com")'`)
	...
	if foundRepos == 0 {
		fmt.Println("No GitLab repositories found with URL 'https://gitlab.com'")
	} else {
