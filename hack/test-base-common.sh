...
# Optional shortcut cross-check: original global base, if present.
if [ -f "$ORIG_DIR/base/kustomization.yaml" ]; then
  echo "Note: original global base exists ($ORIG_DIR/base); comparing rd base against it."
  if split_build "$ORIG_DIR/base" "origbase"; then
    ...
    if [ -z "$d" ]; then
      ...
      echo "  rd base DIFFERS from original base:"
      n="$(printf '%s\n' "$d" | wc -l)"
      if [ "$n" -gt "$MAX_DIFF_LINES" ]; then
        printf '%s\n' "$d" | head -n "$MAX_DIFF_LINES" | sed 's/^/    /'
