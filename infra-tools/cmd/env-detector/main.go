...
func main() {
	...
	if len(changedFiles) == 0 {
		...
		if !*dryRun {
			// Remove all managed labels since nothing changed
			if err := syncLabels(ctx, *githubToken, *repo, *prNumber, nil); err != nil {
...
// printDryRun prints the detection results in a human-friendly format.
func printDryRun(result *detector.Result, labels []string) {
	fmt.Println("\nChanged files:")
