...
func main() {
	var (
		...
		prNumber    = flag.Int("pr-number", 0, "PR number for posting comments (ci-comment mode)")
		githubRepo  = flag.String("github-repo", "", "GitHub repository in owner/repo format (ci-comment mode)")
		githubToken = flag.String("github-token", "", "GitHub token for API access (ci-comment mode, default: $GITHUB_TOKEN)")
		dryRun      = flag.Bool("dry-run", false, "Print output without posting to GitHub")
	...
	case OutputModeCIComment:
		...
		runCIComment(ctx, engine, affected, headSHA, baseSHA, *prNumber, *githubRepo, token, *dryRun)
...
// runCIComment generates the PR comment markdown and optionally posts it to GitHub.
func runCIComment(ctx context.Context, engine *renderdiff.Engine, affected map[detector.Environment][]appset.ComponentPath, headSHA, baseSHA string, prNumber int, githubRepo, githubToken string, dryRun bool) {
	result, err := engine.Run(affected)
	if err != nil {
		logging.Fatal("render-diff failed", "err", err)
	}
	...
	// Dry-run or no PR info: print to stdout
	if dryRun || prNumber == 0 || githubRepo == "" || githubToken == "" {
		fmt.Print(body)
		return
	}

	// Post to GitHub
	client, err := ghclient.NewCommentClient(githubToken, githubRepo)
	if err != nil {
