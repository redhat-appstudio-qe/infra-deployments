...
func main() {
	...
	// Validate flag values early, before any heavy work.
	mode := OutputMode(*outputMode)
	switch mode {
	case OutputModeLocal, OutputModeCISummary, OutputModeCIComment, OutputModeCIArtifact:
		// valid
	default:
		fmt.Fprintf(os.Stderr, "invalid --output-mode %q: must be one of local, ci-summary, ci-comment, ci-artifact-dir\n", *outputMode)
		os.Exit(1)
	}
